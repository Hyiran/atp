(function () {
    ...
    d.AmStockChart = d.Class({
        construct: function (a) {
            ...
            var b = this.theme;
            if (this.listeners)for (var c in this.listeners) {
            ...
            if (f && l) {
                ...
                c.endDate = b;
                p && p.zoom(a, b);
                c.skipEvents || a.getTime() == c.previousStartDate.getTime() && b.getTime() == c.previousEndDate.getTime() || (p = {type: "zoomed"}, p.startDate = a, p.endDate = b, p.chart = c, p.period = k, c.fire(p.type, p), c.previousStartDate = d.newDate(a), c.previousEndDate = d.newDate(b))
            }
    ...
    d.PeriodSelector = d.Class({
        construct: function (a) {
            ...
            if (b.inputFieldsEnabled) {
                ...
                h.appendChild(g);
                b.startDateField = g;
                if (f)m = b.width - 6 + "px", d.addBr(h); else {
            ...
            for (p = 0; p < r; p++) {
                ...
                for (t = m; t < g; t++) {
                    ...
                    if (t == m || E >= x[u] || !x[u]) {
                        ...
                        y[u][h] = d.newDate(B, b)
                    } else for (D = 0; D < z; D++)v = n[D].toField, w = y[u], p == r - 1 && (w[h] = d.newDate(B, b)), q = Number(C[v]), isNaN(q) || (isNaN(w[v + "Low"]) && (w[v + "Low"] = q), q < w[v + "Low"] && (w[v + "Low"] = q), isNaN(w[v + "High"]) && (w[v + "High"] = q), q > w[v + "High"] && (w[v + "High"] = q), isNaN(w[v + "AbsHigh"]) && (w[v + "AbsHigh"] = q), Math.abs(q) > w[v + "AbsHigh"] &&
    ...
    d.parseEvents = function (a, b, c, e, f, l) {
        ...
        for (var E in h)if (h.hasOwnProperty(E)) {
            ...
            for (v = 0; v < t; v++) {
                ...
                for (g = 0; g < m; g++)for (y = b[g], x = y.graphs, r = x.length,
                                                p = 0; p < r; p++) {
                    u = x[p];
                    var q = {};
                    q.eventDispatcher = f;
...
(function () {
    ...
    d.DataSetSelector = d.Class({
        ...
        write: function (a) {
            ...
            if (0 < b.countDataSets("showInCompare") && 1 < l.length)if (k ? (d.addBr(a), d.addBr(a)) : (c = document.createTextNode(" "), a.appendChild(c)), c = document.createTextNode(d.lang.compareText || b.compareText), a.appendChild(c), g = b.listCheckBoxSize, k) {
                d.addBr(a);
